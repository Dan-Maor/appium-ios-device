...
async function connectPortSSL (udid, port, socket, callback) {
  ...
  try {
    ...
    const upgrade = (socket) => upgradeToSSL(socket, pairRecord.HostPrivateKey, pairRecord.HostCertificate);
    return await usbmux.connect(device.Properties.DeviceID, port, (socket) => callback ? callback(upgrade(socket)) : upgrade(socket));
  } catch (e) {
    usbmux.close();
    throw e;
  }
